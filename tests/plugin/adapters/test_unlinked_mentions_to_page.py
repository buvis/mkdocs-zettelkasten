...
class TestAdaptUnlinkedMentionsToPage:
    def test_skips_non_zettel(self) -> None:
        page = _make_page(is_zettel=False)
        mentions = {}
        adapt_unlinked_mentions_to_page(page, mentions, lambda x: None)
    ...
    def test_adds_mention_to_target(self) -> None:
        ...
        adapt_unlinked_mentions_to_page(page, mentions, zettel_lookup)
    ...
    def test_skips_when_zettel_meta_missing(self) -> None:
        ...
        adapt_unlinked_mentions_to_page(page, {99: [(1, "snippet")]}, lambda x: None)

    def test_empty_mentions_dict(self) -> None:
        page = _make_page(zettel_id=1)
        target = MagicMock()
        target.unlinked_mentions = []
        adapt_unlinked_mentions_to_page(page, {}, MagicMock(return_value=target))
        assert len(target.unlinked_mentions) == 0
