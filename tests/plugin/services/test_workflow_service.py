...
class TestComputeWithoutTimezoneOrToday:
    def test_raises_without_timezone_or_today(self):
        svc = WorkflowService()
        store = ZettelStore([])
        with pytest.raises(RuntimeError, match="requires either today or timezone"):
            svc.compute(store, {}, {})
...
class TestInboxTimezoneBoundary:
    ...
    def test_fleeting_stale_in_utc_but_not_in_local(self):
        ...
        with patch(
            ...
            mock_dt.now = _patched_now(utc_instant)
            mock_dt.side_effect = datetime
            result_utc = svc_utc.compute(store, {}, {}, timezone=_UTC)
